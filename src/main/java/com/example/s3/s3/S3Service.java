...
import com.example.s3.s3.ImageRepository;
...
public class S3Service {
    ...
    @Autowired
    public S3Service(AmazonS3 amazonS3, @Value("${cloud.aws.s3.bucket}") String bucket, ImageRepository imageRepository) {
    ...
    private void saveFileMetadata(String originalFileName, String uniqueFileName, long fileSize, String contentType) {
        com.example.s3.s3.Image image = new Image();
        image.setOriginalFileName(originalFileName);
        image.setUniqueFileName(uniqueFileName);  // Save full path including directory
        image.setFilePath(uniqueFileName);        // Store path to match S3 key
